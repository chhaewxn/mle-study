...
---
> **데이터 설계는 기술이 아니라, 모델 워크로드에 맞춰 데이터를 조리하는 일이다.**
> 

### 3.4 데이터 스토리지 엔진 및 처리

**데이터베이스는 “저장”이 아니라 “워크로드 최적화”**

데이터베이스(스토리지 엔진)는 단순히 데이터를 담아두는 공간이 아니라, 데이터가 **어떻게 저장되고, 어떻게 읽히고, 어떻게 갱신되는지**를 구현. DB 선택의 기준은 보통 아래 두 워크로드 중 **업무가 어디에 최적화되어 있는지**가 중요함!

- **트랜잭션 처리(OLTP 성격)**: 잦은 쓰기/갱신, 낮은 지연시간, 높은 가용성
- **분석 처리(OLAP 성격)**: 대량 스캔/집계, 다양한 관점의 조회, 열 지향/압축/벡터화 등

→ **“어떤 DB가 좋은가?”보다 “우리의 읽기/쓰기 패턴이 무엇인가?”가 우선!**

### 3.4.1 트랜잭션 처리와 분석 처리

**트랜잭션(OLTP)의 요구사항: 낮은 레이턴시 + 고가용성**

트랜잭션은 생성(INSERT)되고, 변경(UPDATE)되고, 필요 시 삭제(DELETE)됨. 사용자-facing 작업이 많아 **빠르고 안정적으로 처리**되어야 함

**ACID는 대표적인 설계 목표(절대 조건은 아님)**

- **Atomicity(원자성)**: 트랜잭션은 전부 성공하거나 전부 실패
- **Consistency(일관성)**: 정의된 규칙/제약을 위반하지 않음
- **Isolation(격리성)**: 동시 트랜잭션이 서로 간섭하지 않도록 보장
- **Durability(지속성)**: 커밋 이후 장애가 나도 결과가 유지

다만 현실의 트랜잭션 DB는 **ACID를 완화하거나 다른 방식으로 재해석해 가용성과 확장성을 확보**하기도 함

**(3) 분석(OLAP)의 요구사항: 대량 집계와 조회 효율성**

분석 질문은 보통 “여러 행에 걸친 열 데이터를 집계”하는 형태라, 트랜잭션 DB가 비효율적인 경우가 많음. 그런데 분석 DB는 이런 집계를 빠르게 하기 위한 구조적 최적화를 가짐.

**OLTP vs OLAP 용어가 예전만큼 딱 나뉘지 않는 이유**

문서는 전통적인 구분이 흐려지는 이유는?

- **트랜잭션/분석을 모두 다루는 엔진 등장**(경계가 약해짐)
- **컴퓨트(처리)와 스토리지 분리 패러다임 확산**
    - 동일한 데이터 저장소 위에, 처리 계층을 달리해서 다양한 쿼리를 해결
- 온라인이라는 말의 의미가 넓어져서(즉시 처리/니어라인/오프라인 등) 문맥 분리가 필요

### 3.4.2 ETL vs ELT — 데이터 파이프라인의 기본기
...
- **Extract**
    ...
    - 초기 검증, 필터링, 거부 처리 가능
- **Transform**
    - 정제, 조인, 표준화, 중복 제거, 집계, 피처 생성 등 대부분의 로직
- **Load**
    - 대상 저장소에 적재 방식과 주기 결정
> 초기 추출 단계에서의 검증은 다운스트림 비용을 크게 줄일 수 있음

**(2) ELT (Extract → Load → Transform)**
- 원시 데이터를 먼저 저장소에 적재
- 이후 필요 시점에 변환 수행
- 장점: 초기 적재 속도 빠르고 스키마 변경에 유연
- 단점: 데이터가 커질수록 필요한 데이터만 선별·처리하는 비용 증가 → 비효율적일 수 있음

**(3) 현실: 레이크/웨어하우스의 절충(레이크하우스 등)**
- 데이터 규모 증가와 스키마 진화로 인해, 유연성(데이터 레이크)과 관리/성능(웨어하우스)을 결합한 구조가 확산되고 있음

---

### 3.5 데이터플로(프로세스 간 데이터 전달) 3가지 모드

**(1) 데이터베이스를 통한 전달**
- A가 DB에 쓰고, B가 읽는 가장 단순한 방식
- 한계:
    - 동일 DB 접근 불가한 환경
    - 레이턴시 요구가 엄격한 서비스엔 느릴 수 있음

**(2) 서비스를 통한 전달 (REST / RPC)**
- 네트워크 요청을 통한 직접 전달
- 마이크로서비스 아키텍처에서 흔함
- 한계:
    - 동기 호출 시 장애 전파(의존 서비스 다운 → 호출 서비스도 영향)
    - 서비스 수가 많아질수록 호출 그래프가 복잡해짐

**(3) 실시간 전송을 통한 전달(이벤트 기반: Kafka/Kinesis, 메시지 큐 등)**
- 서비스 간 “직접 요청” 대신 브로커를 통해 이벤트를 발행/구독
- 장점: 서비스 간 결합도 감소, 확장성 및 탄력성 향상, 최신 상태 공유에 유리
- 개념 정리:
    - **이벤트 버스 / 이벤트 기반 아키텍처**
    - **pub-sub**: 토픽 기반으로 다수가 구독 가능, 리텐션 정책 존재 가능
    - **메시지 큐**: 특정 소비자/소비자 그룹 중심으로 전달되는 성격이 강함

---

### 3.6 배치 처리 vs 스트림 처리 — 피처/지연시간 관점

**배치 처리(Batch)**
- 과거 데이터(누적 데이터)를 주기적으로 처리(일 단위 등)
- 비교적 덜 자주 변하는 피처(정적/배치 피처)에 적합

**스트림 처리(Stream)**
- 데이터가 생성되자마자(또는 매우 짧은 주기) 처리
- 레이턴시가 짧고 “현재 상태” 기반의 피처(동적/스트리밍 피처)에 적합
- 상태 유지(stateful) 계산이 강점이 될 수 있음(누적을 매번 다시 계산하지 않는 방식)

**실무 포인트**
- 많은 ML/분석 시스템은 **배치 + 스트림 피처를 함께** 필요로 함
- 스트림 피처가 많아지면 단순 브로커 내장 기능만으로는 부족해지고, 전용 스트림 처리 엔진/SQL 추상화가 필요해짐(책에서 예시 언급)

→ **결론**: 데이터 시스템 설계를 **워크로드(트랜잭션 vs 분석), 파이프라인 패턴(ETL/ELT), 데이터 전달 방식(DB/서비스/이벤트), 처리 모드(배치/스트림)** 정리하며, 현대에는 경계가 흐려지고(컴퓨트-스토리지 분리, 하이브리드 DB/레이크하우스 등) **요구사항 기반의 선택**이 더욱 중요하다는 메세지를 전달
...
**Q6. 서비스 간 데이터 전달을 DB로 할지, API로 할지, 이벤트(Kafka 등)로 할지 어떻게 판단하는가?**

A. 결합도와 레이턴시, 장애 전파 위험을 기준으로 볼 수 있습니다.
- DB 전달: 가장 단순하지만, 레이턴시/공유 접근 제약이 크고 운영 경계가 불명확해질 수 있습니다.
- 요청 기반(API/RPC): 즉시성이 좋지만 동기 의존이 늘면 장애 전파가 커집니다.
- 이벤트 기반(pub-sub/메시지큐): 생산자-소비자 결합도를 낮추고 확장에 유리하지만, 이벤트 스키마/리텐션/재처리/정확히-한번 처리 등 운영 고려사항이 늘어납니다.

**Q7. 배치 처리와 스트림 처리를 함께 써야 하는 기준은 무엇인가?**

A. 피처/지표가 “얼마나 자주 변하고, 얼마나 빨리 반영되어야 하는가”로 나눕니다. 다만 대부분의 ML/분석 시스템은 두 종류가 섞이므로, 정합성(배치 vs 스트림 결과의 불일치) 관리까지 포함해 설계합니다.

- 배치: 하루/시간 단위로 충분한 정적 피처(누적 통계, 비교적 느리게 변하는 속성)
- 스트림: 분/초 단위로 바뀌는 동적 피처(현재 상태, 최근 N분 이벤트 기반 집계)
